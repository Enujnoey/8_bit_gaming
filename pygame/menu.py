...
class MainMenu(Menu):
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.game.draw_text("Stranded", 40, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 -40)
...
class OptionsMenu(Menu):
    ...
    def display_menu(self):
        self.run_dispplay = True
        self.game.font_name = self.game.font_title #need to clarify the font
        while self.run_display:
            self.game.check_events()
            self.check_input()
            self.game.screen.fill(self.game.BLACK)
    ...
    def check_input(self):
        if self.game.BACK_KEY:
            ...
            if self.state == "Volume":
                self.state = "Controls"
                self.cursor_rect.midtop: (self.controlsx + self.offset, self.coltrolsy)

            elif self.state == "Controls":
                self.state = "Volume"
                self.cursor_rect.midtop = (self.volx + self.offset, self.voly)
        elif self.game.START_KEY:
            #create a volume / controls menu and implement a control
            pass
